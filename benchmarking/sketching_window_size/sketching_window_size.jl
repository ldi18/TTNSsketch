...
function run_benchmark(; max_n::Int = 12,
                       ...
                       n_runs::Int = 5,
                       base_seed::Int = 1234)
  results_path = joinpath(@__DIR__, "sketching_window_size_results.csv")
  ...
  output_pdf_error = joinpath(@__DIR__, "sketching_window_size_error_vs_n.pdf")
  savefig(plt_error, output_pdf_error)
  println("Error plot saved to: $output_pdf_error")
  
  output_pdf_runtime = joinpath(@__DIR__, "sketching_window_size_runtime_vs_n.pdf")
  if plt_runtime !== nothing
    savefig(plt_runtime, output_pdf_runtime)
    println("Runtime plot saved to: $output_pdf_runtime")
  end
  
  output_pdf_combined = joinpath(@__DIR__, "sketching_window_size_combined.pdf")
...
if abspath(PROGRAM_FILE) == @__FILE__
  ...
  println("  Plots saved:")
  println("    - sketching_window_size_error_vs_n.pdf")
  println("    - sketching_window_size_runtime_vs_n.pdf")
  println("    - sketching_window_size_combined.pdf")
  println("="^60)
end
